...
DECLARE_SPEC_FUN(	spec_mage_guardian	);
...
DECLARE_SPEC_FUN(	spec_area_guardian	);
...
SPEC_FUN *spec_lookup( const char *name )
{
    ...
    if ( !str_cmp( name, "spec_mage_guardian"     ) ) return spec_mage_guardian;
    ...
    if ( !str_cmp( name, "spec_area_guardian"     ) ) return spec_area_guardian;
...
/*
 * Given a SPEC_FUN, return the spec description (For hqedit and mstat)
 */
char * get_spec_desc( SPEC_FUN *spec )
{
    if ( spec == spec_breath_any 	) return "spec_breath_any";
    if ( spec == spec_breath_acid	) return "spec_breath_acid";
    if ( spec == spec_breath_fire	) return "spec_breath_fire";
    if ( spec == spec_breath_frost	) return "spec_breath_frost";
    if ( spec == spec_breath_gas	) return "spec_breath_gas";
    if ( spec == spec_breath_lightning	) return "spec_breath_lightning";

    if ( spec == spec_cast_adept	) return "spec_cast_adept";
    if ( spec == spec_cast_cleric	) return "spec_cast_cleric";
    if ( spec == spec_cast_judge	) return "spec_cast_judge";
    if ( spec == spec_cast_mage		) return "spec_cast_mage";
    if ( spec == spec_cast_undead	) return "spec_cast_undead";

    if ( spec == spec_fido		) return "spec_fido";
    if ( spec == spec_guard		) return "spec_guard";
    if ( spec == spec_janitor		) return "spec_janitor";
    if ( spec == spec_mayor		) return "spec_mayor";
    if ( spec == spec_poison		) return "spec_poison";
    if ( spec == spec_thief		) return "spec_thief";

    if ( spec == spec_eater		) return "spec_eater";
    if ( spec == spec_gremlin_original	) return "spec_gremlin_original";
    if ( spec == spec_gremlin_born	) return "spec_gremlin_born";
    if ( spec == spec_rogue		) return "spec_rogue";
    if ( spec == spec_zombie_lord	) return "spec_zombie_lord";
    if ( spec == spec_dog		) return "spec_dog";
    if ( spec == spec_decap		) return "spec_decap";
    if ( spec == spec_clan_guardian	) return "spec_clan_guardian";
    if ( spec == spec_mage_guardian	) return "spec_mage_guardian";
   
    perror("get_spec_desc: bad spec");
...
//    ch->pIndexData->spec_name = str_dup( "spec_cast_adept" );
...
//    ch->pIndexData->spec_name = str_dup( "spec_cast_cleric");
...
bool spec_cast_undead( CHAR_DATA *ch )
{
	...
	default: min_level = 21; spell = "teleport";       break;
	}
...
bool spec_eater( CHAR_DATA *ch )
{
   ...
   */
   ...
   if ( (ch->position != POS_FIGHTING) || IS_SET(ch->in_room->room_flags,
ROOM_NO_DECAP)|| IS_SET(ch->in_room->room_flags, ROOM_NO_RECALL))
   {
      return FALSE;
   }
/*   if (number_percent() > 50 ) return FALSE;*/
   ...
   if (IS_AFFECTED(victim, AFF_PARADOX) ) return FALSE;
...
//   ch->pIndexData->spec_name = str_dup( "spec_clan_guardian");
   ...
   strcpy(clanname, ch->in_room->area->name);
   clanname[strlen(clanname) - 3] = '\0';
   ...
   {
       if (!IS_PLAYING(d)
	   || (victim = (d->character ? d->character : d->original)) == NULL
           || IS_NPC(victim)
           || IS_IMMORTAL(victim)
           || victim->pcdata->chobj != NULL
	   || victim->in_room == NULL
           || victim->in_room->area != ch->in_room->area
           || (victim->clan != NULL && !str_cmp(victim->clan,clanname) ) )
           {
             continue;
           }

/*        if ( IS_EXTRA(victim, EXTRA_OSWITCH) ) do_humanform(victim,"");*/


        if (ch->in_room != victim->in_room)
        {
           act("#R$n disappears in a haze of red!",victim,NULL,NULL,TO_ROOM);
           act("#RYou disappear into a haze of red!",victim,NULL,NULL,TO_CHAR);

           char_from_room( victim );
           char_to_room(victim, ch->in_room);

           act("#R$n appears in a haze of red!",victim,NULL,NULL,TO_ROOM);
           act("#RYou appear into a haze of red!",victim,NULL,NULL,TO_CHAR);
        }
     
	   if (IS_AFFECTED(victim,AFF_SHADOWPLANE) ) do_shadowplane(victim,"");
	   if (IS_POLYAFF(victim, POLY_MIST))
	   {
		REMOVE_BIT(victim->affected_by, AFF_ETHEREAL);
	   }

        act("#R$n shouts at you \"You shall DIE!\"",ch,ch->long_descr,victim,TO_VICT);
        act("You let out a battlecry as you defend the clan headquaters!",ch,NULL,victim,TO_CHAR);
/*
        spell_curse( 24 , 100 , ch , victim );
*/
        if (victim->hit <= 200 && number_range(1,5) != 1)
        {
           continue;
        }
        sprintf( buf,"#R%s Is In %s Headquarters! Attack!#n", victim->name, clanname); 
        do_shout( ch , buf);
...
bool spec_mage_guardian(CHAR_DATA *ch)
{
   DESCRIPTOR_DATA  *d;
   CHAR_DATA        *victim;
   char             buf[MAX_STRING_LENGTH];
   sh_int	    mage_color = 0;  /* Defaults to purple mage */

    if (!str_cmp(ch->in_room->area->name, "Red Mage HQ") )
	mage_color = 1;
    else if (!str_cmp(ch->in_room->area->name, "Blue Mage HQ") )
	mage_color = 2;
    else if (!str_cmp(ch->in_room->area->name, "Green Mage HQ") )
	mage_color = 3;
    else if (!str_cmp(ch->in_room->area->name, "Yellow Mage HQ") )
	mage_color = 4;
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
	   ...
	   || (IS_CLASS(victim, CLASS_MAGE) &&
		victim->pcdata->powers[MPOWER_RUNE0] == mage_color ) )
           {
             continue;
           }
        ...
        if (ch->in_room != victim->in_room)
        {
           act("#R$n disappears in a haze of red!#n",victim,NULL,NULL,TO_ROOM);
           act("#RYou disappear into a haze of red!#n",victim,NULL,NULL,TO_CHAR);

           char_from_room( victim );
           char_to_room(victim, ch->in_room);

           act("#R$n appears in a haze of red!#n",victim,NULL,NULL,TO_ROOM);
           act("#RYou appear into a haze of red!#n",victim,NULL,NULL,TO_CHAR);
        }
	   ...
	   if (IS_POLYAFF(victim, POLY_MIST))
	   {
        ...
        act("#R$n shouts at you \"You shall DIE!\"#n",ch,ch->long_descr,victim,TO_VICT);
        act("You let out a battlecry as you defend the clan headquaters!",ch,NULL,victim,TO_CHAR);
        ...
        sprintf( buf,"#R%s has invaded!  Attack!#n\n\r", victim->name); 
        do_shout( ch , buf);
...
bool spec_area_guardian(CHAR_DATA *ch)
{
   DESCRIPTOR_DATA       *d;
   CHAR_DATA             *victim;
   char                  buf[MAX_STRING_LENGTH];
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
           ...
           || victim->in_room == ch->in_room)
	{
        ...
        act("You let out a battlecry as you defend this area!",ch,NULL,victim,TO_CHAR);
...
/*        if (victim->position <= POS_MORTAL && IS_HERO(victim) &&
	  ch->position == POS_STANDING)
        ...
        if (victim->position <= POS_MORTAL && IS_HERO(victim) && ch->position == POS_STANDING )
        {
	 ...
	 sprintf( buf,"%s has been decapitated by %s.",victim->name, ch->short_descr);
	 do_info(ch, buf);
	 victim->pcdata->stats[UNI_RAGE] = 0;
         if (victim->level > 2 && victim->level < 7)
	     victim->level = 2;
...
*/
        ...
        sprintf( buf,"%s Is in my Headquarters! Attack!\n\r", victim->name); 
        do_shout( ch , buf);
...
bool spec_zombie_lord( CHAR_DATA *ch )
{
	...
	else if (!is_affected(ch,skill_lookup("speed")) && number_percent() < 50)
	{
	    act("$n's eyes glow bright blue for a moment.",ch,NULL,NULL,TO_ROOM);
	    spell_speed( skill_lookup( "speed" ), ch->level, ch, ch );
	}
	else if (!is_affected(ch,skill_lookup("combat mind")) && number_percent() < 50)
	{
	    act("$n's eyes glow bright blue for a moment.",ch,NULL,NULL,TO_ROOM);
	    spell_combat_mind( skill_lookup( "combat mind" ), ch->level, ch, ch );
	}
	...
	else
	{
        ...
        if (victim->position <= POS_MORTAL && IS_HERO(victim) && ch->position == POS_STANDING  && !IS_NPC(victim))
        {
...

{
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
           ...
           || victim->in_room == ch->in_room)
	{
        ...
        sprintf( buf,"HELP! %s is trying to molest my children!\n\r",
        victim->name); 
        do_shout( ch , buf);
