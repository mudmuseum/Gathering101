...
void do_immune( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_IMMUNE(ch,IMM_HIGHLANDER)) send_to_char(" Highlander",ch);
    }
    else
	send_to_char(" None",ch);
    send_to_char(".\n\r",ch);
...
void show_spell( CHAR_DATA *ch, int dtype )
{
    ...
    else if (ch->spl[dtype] <= 275 && IS_CLASS(ch, CLASS_MAGE)) sprintf(bufskill,"at one with");
    else if (ch->spl[dtype] >= 300 && IS_CLASS(ch, CLASS_MAGE)) sprintf(bufskill,"completely in tune with");
    else return;
    if      (dtype == 0)
