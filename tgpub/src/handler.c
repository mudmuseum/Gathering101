...
/* 
 * Name shortening version of is_name, must type at least 3 chars 
 */
bool is_name ( char *str, char *namelist )
{
    ...
    for ( ; ; )  /* start parsing string */
    {
	    ...
	    if ( (strlen(part) < 3) && (strlen(name) > 2) ) continue;
...
*/
    ...
    case APPLY_SAVING_SPELL:  ch->saving_throw		+= mod;	break;
    case APPLY_POLY:	      ch->polyaff		+= mod;	break;
    case APPLY_DAMCAP:	      ch->damcap[DAM_MOD]	+= mod; break;
    }
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ((IS_NPC(ch) || (!IS_CLASS(ch, CLASS_DEMON))) &&
	IS_SET(obj->spectype, SITEM_DEMON))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	...
	IS_SET(obj->spectype, SITEM_WEREWOLF))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	...
	IS_SET(obj->spectype, SITEM_VAMPIRE))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
 ...
 */
    if (IS_SET(obj->quest, QUEST_UNIQUE) && IS_ITEMAFF(ch, ITEMA_UNIQUE))
    {
	act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	...
	else if ((sn == 52 || sn == 6) && !IS_AFFECTED(ch, AFF_PASS_DOOR))
	{
	...
	else if ((sn == 54 || sn == 7) && !IS_AFFECTED(ch, AFF_PROTECT))
	{
	...
	else if ((sn == 57 || sn == 8) && !IS_AFFECTED(ch, AFF_SANCTUARY))
	{
	...
	else if (sn == 26 && !IS_NPC(ch) && !IS_ITEMAFF(ch, ITEMA_BODYREGEN))
	{
	    SET_BIT(ch->itemaffect, ITEMA_BODYREGEN);
	}
	else if (sn == 28 && !IS_NPC(ch) && !IS_ITEMAFF(ch, ITEMA_DEFLECT))
	{
	    SET_BIT(ch->itemaffect, ITEMA_DEFLECT);
	    send_to_char("You are engulfed in a shimmering magickal force.\r\n", ch);
	    act("$n is engulfed in a shimmering magickal force.", ch, NULL, NULL, TO_ROOM);
	}

	if (IS_SET(obj->spectype, SITEM_DEMONIC) && !IS_NPC(ch))
	    if (ch->pcdata->stats[DEMON_POWER] < 17)
		ch->pcdata->stats[DEMON_POWER] += 1;
	if (IS_SET(obj->spectype, SITEM_HIGHLANDER) && !IS_NPC(ch) && IS_HERO(ch) && obj->item_type == ITEM_WEAPON)
	    SET_BIT(ch->itemaffect, ITEMA_HIGHLANDER);
	if (IS_SET(obj->quest, QUEST_ARTIFACT))
	    SET_BIT(ch->itemaffect, ITEMA_ARTIFACT);
	if (IS_SET(obj->quest, QUEST_UNIQUE))

	    SET_BIT(ch->itemaffect, ITEMA_UNIQUE);
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	else if (sn == 25 && !IS_NPC(ch) && IS_ITEMAFF(ch, ITEMA_RAGER))
	{
	    ...
	    REMOVE_BIT(ch->itemaffect, ITEMA_BODYREGEN);
	else if (sn== 28 && !IS_NPC(ch) && IS_ITEMAFF(ch, ITEMA_DEFLECT))
	    REMOVE_BIT(ch->itemaffect, ITEMA_DEFLECT);
	if (IS_SET(obj->spectype, SITEM_DEMONIC) && !IS_NPC(ch)
	...
	if (IS_SET(obj->quest, QUEST_ARTIFACT)) 
        ...
        if (IS_SET(obj->quest, QUEST_UNIQUE))
            REMOVE_BIT(ch->itemaffect, ITEMA_UNIQUE);
...
bool break_hold_person( CHAR_DATA *victim )
{   
   AFFECT_DATA *paf;
   int level = 0, strength = 25, save = 0;
       
   for( paf=victim->affected;paf;paf=paf->next)
   {
      if( paf->type == gsn_hold_person)
         break;
   }
    
   level = paf->duration * 2;
   
   if( IS_NPC(victim))
      strength= 20;
   else
      strength = get_curr_str(victim);
   ...
   for( paf=victim->affected;paf;paf=paf->next)
   {
      ...
      {
       if( strength >= 17 )
        paf->duration = UMAX(1, paf->duration - 2);
       else
         paf->duration = UMAX(1, paf->duration - 1);
      }
   }
   
   save = victim->level / 3 + (strength - 11 ) - level;
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( !IS_NPC(ch) && IS_SET(ch->act, AFF_INFRARED) )
	return TRUE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_DAMCAP:		return "maximum damage";
    }
...
OBJ_DATA *carrying_quest_bit ( CHAR_DATA *ch, int search_bit)
{
    OBJ_DATA *obj;
    OBJ_DATA *obj2;
    ...
    for ( obj = ch->carrying; obj != NULL; obj = obj->next_content )
    {
	if ( IS_SET(obj->quest, search_bit) )
	    return obj;

   	obj2 = contains_quest_bit(obj, QUEST_ARTIFACT);
...
OBJ_DATA *contains_quest_bit ( OBJ_DATA *obj, int search_bit )
{
    OBJ_DATA *obj2;
	...
	if ( IS_SET(obj2->quest, search_bit) )
	    return obj2;
