...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
    ...
    if (ch->spl[dtype] >= 240 && (ch->level == 4)) return;
    if (ch->spl[dtype] >= 275 && (ch->level == 5)) return;
    if (ch->spl[dtype] >= 300 && (ch->level == 6)) return;
    if ((dice1 > ch->spl[dtype] || dice2 > ch->spl[dtype]) || (dice1==100
|| dice2>=100)) ch->spl[dtype] += 1;
    ...
    else if (ch->spl[dtype] == 276) sprintf(bufskill,"at one with");
    ...
    sprintf(buf,"#WYou are now %s %s magic.#n\n\r",bufskill,buftype);
    send_to_char(buf,ch);
    return;
    }
}
...
bool saves_spell( int level, CHAR_DATA *victim )
{
	...
	tsave = ( victim->spl[0]+victim->spl[1]+victim->spl[2]+
    	...
    	save = 50 + ( tsave - level - char_save(victim) ) * 5;
    }
    else
    	save = 50 + ( victim->level - level - char_save(victim) ) * 5;
    save = URANGE( 15, save, 85 );
    
    return number_percent( ) < save;
}

bool saves_skill ( CHAR_DATA *ch, CHAR_DATA *victim )
{
    int vict_save = 0;

    if ( !IS_NPC(victim) )
    {
	/* Stance bonus, more if it's a defensive stance */
	if ( (victim->stance[0] == STANCE_SWALLOW ||
	   	victim->stance[0] == STANCE_MANTIS) &&
		ch->stance[0] != STANCE_MONKEY )
	    vict_save += 25;
	else if (victim->stance[0] > 0)
	    vict_save += 15;

	/* Stance skill bonus GM = +25 */
	vict_save += victim->stance[victim->stance[0]] / 8;
		
	if (IS_CLASS(victim, CLASS_HIGHLANDER) &&
	   victim->pcdata->powers[HPOWER_TRAINED] >= 7)
	    vict_save += number_range(10, 20);

	vict_save += number_range(1,15);
    }
    else
	vict_save = UMIN(victim->level, 75);

    vict_save = URANGE(25, vict_save, 75);

    return number_percent() < vict_save;
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->in_room == get_room_index(ROOM_VNUM_PUNISHMENT) )
    {
	...
	{
		send_to_char("You dont see that here?\n\r", ch);
		return;
	}
...
*/
    ...
    if (ch->level < 7) WAIT_STATE( ch, skill_table[sn].beats );
...
void spell_hold_person(int sn, int level, CHAR_DATA * ch, void *vo)
{
   ...
   {
      act("$N is already affected by that spell!",ch,ch,victim,TO_CHAR);
      return;
   }
   if (saves_spell(level, victim) )
   {
      act("$N shrugs off your spell!",ch,ch,victim,TO_CHAR);
      return;
   }
   ...
   act("You capture $N with your spell.",ch,ch,victim,TO_CHAR);     
   act("$n's spell slows $N down.\n\r", ch, NULL, victim, TO_NOTVICT);
   act("$n's spell slows you down!\n\r", ch, NULL, victim, TO_VICT);
   return;
}

void spell_speed(int sn, int level, CHAR_DATA * ch, void *vo)
{
   CHAR_DATA *victim = (CHAR_DATA *) vo;
   AFFECT_DATA af;
   ...
   if (is_affected(victim, sn) )
   {
      send_to_char("You can't go any faster.\n\r", ch );
      return;
   }
   ...
   send_to_char("You speed your atoms.\n\r", ch);
   return;
}
...
void spell_combat_mind(int sn, int level, CHAR_DATA * ch, void *vo)
{
   ...
   if (victim != ch)
    ...
    act("You give $N a better understanding of battle tactics.\n\r", ch, NULL, victim, TO_CHAR);
     }
...
void spell_complete_healing(int sn, int level, CHAR_DATA * ch, void *vo)
{
   ...
   if (ch != victim)
      {
	act("Your spell completely heals $N.\n\r", ch, NULL, victim, TO_CHAR);
	send_to_char("Ahhhhhh...You are completely healed!\n\r", victim);
	act("$n's spell completely heals $N.\n\r", ch, NULL, victim, TO_NOTVICT);
      }
   return;
}
...
void spell_enhance_armor(int sn, int level, CHAR_DATA * ch, void *vo)
{
   ...
   /* Good enhancement */
   {
	 ...
	 act("$p in $n's hand glows blue.\n\r", ch, obj, NULL, TO_ROOM);
      }
      else if (IS_EVIL(ch))
      {
	 SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
	 act("$p glows red.\n\r", ch, obj, NULL, TO_CHAR);
	 act("$p in $n's hand glows red.\n\r", ch, obj, NULL, TO_ROOM);
      }
      else
      {
	 SET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
	 SET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
	 act("$p glows yellow.\n\r", ch, obj, NULL, TO_CHAR);
	 act("$p in $n's hand glows yellow.\n\r", ch, obj, NULL, TO_ROOM);
      }
...
void spell_animate_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
    victim=create_mobile( get_mob_index( MOB_VNUM_WALKING_DEAD) );
    victim->level = 100;
    victim->hit = 10000;
    ...
    victim->armor = -1000;
	  ...
	  SET_BIT(victim->affected_by, AFF_SNEAK);
    ...
    send_to_char( "A ghost raises from the corpse and vanishes to another plane.\n\r",ch);
    act( "$N raises from the corpse, screams at $n and vanishes to another plane.", ch, NULL, victim, TO_ROOM );
    add_follower( victim, ch );
    return;
    }
    else
    {
    send_to_char( "Now how are you suppose to animate that?\n\r",ch);
    return;
    }
}
...
void spell_gate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    else send_to_char( "The gate just won't come.\n", ch );
}
...
void spell_harm( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam = UMAX(  20, victim->hit - (dice(1,4)/4) );
    if ( saves_spell( level, victim ) )
    dam = UMIN( 50, dam  );
    dam = (UMIN( 100, dam )*4) ;
    damage( ch, victim, dam, sn );
...
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
	if ((ch->level <3) && (victim->level >2))
	{
	send_to_char("A mortal cannot heal an avatar!\n\r",ch);
	return;
	}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_char( "This weapon is powerful in the hands of a Highlander.\r\n", ch);
	...
	if (itemtype == 4)
	    ...
	    sprintf (buf, "This ancient weapon regenerates its wielders lost bodyparts.\n\r");
	else if (itemtype == 28)
	    sprintf(buf, "This powerful weapon deflects damage with magickal force.\r\n");
	else
	...
	if (obj->value[3] == 4)
	    ...
	    sprintf (buf, "This ancient item regenerates its wearers lost bodyparts.\n\r");
	else if (obj->value[3] == 28)
	   sprintf (buf, "This powerful item deflects damage with magickal force.\r\n");
	else
	    sprintf (buf, "This item is bugged...please report it.\n\r");
	if (obj->value[3] > 0)
	    send_to_char( buf, ch );
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = ch->carrying; obj_lose != NULL; obj_lose = obj_next )
	{
		...
		sprintf(buf, "%s's %s is consumed by acid breath.", victim->name, obj_lose->short_descr);
		log_string(buf, obj_lose->oid);
		break;
	    }
	}
    }
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	obj_lose = obj_next )
	{
	    ...
	    extract_obj( obj_lose );
	    sprintf(buf, "%s's %s is consumed by fire breath.", victim->name, obj_lose->short_descr);
	    log_string(buf, obj_lose->oid);
	}
    }
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	obj_lose = obj_next )
	{
	    ...
	    sprintf(buf, "%s's %s is consumed by frost breath.", victim->name, obj_lose->short_descr);
	    log_string(buf, obj_lose->oid);
	    act( msg, victim, obj_lose, NULL, TO_CHAR );
	    extract_obj( obj_lose );
	}
...
void spell_transport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (victim->in_room == get_room_index(ROOM_VNUM_PUNISHMENT))
    {
	send_to_char("They are being punished.\r\n", ch);
	return;
    }
    ...
    sprintf(buf, "Log %s transports %s %s", ch->name, arg1, arg2);
    log_string(buf, obj->oid);
...
void spell_brew( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        send_to_char( "You cannot contain such a powerful spell.\n\r", ch );
        return;
    }
...
void spell_spellproof( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    sprintf(buf,"%s casts spellproof on %s.", ch->name, obj->short_descr);
    log_string(buf, obj->oid);
    return;
}
...
void enhance_stat( int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, int apply_bit, int bonuses, int affect_bit )
{
    ...
    if ( IS_SET(affect_bit, AFF_WEBBED) )
    {
	affect_bit -= AFF_WEBBED;
	affect_bit += AFF_MAGEWEBBED;
    }

	/* Remove mageweb affect if victim is already webbed/magewebbed */
    if ( IS_SET(affect_bit, AFF_MAGEWEBBED) && 
	 ( IS_AFFECTED(victim, AFF_WEBBED) || 
	   IS_AFFECTED(victim, AFF_MAGEWEBBED) ))
	affect_bit -= AFF_MAGEWEBBED;

    else if (IS_SET(affect_bit, AFF_MAGEWEBBED) && is_safe(ch, victim))
	affect_bit -= AFF_MAGEWEBBED;
...
void spell_resistance( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    sprintf(buf,"%s casts resistence on %s.", ch->name, obj->short_descr);
    log_string(buf, obj->oid);
    return;
}
...
void spell_web( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_skill( ch, victim ) && victim->position >= POS_FIGHTING )
    {
...
/* kavirpoint Re-enabled by GFrindo to check out what is wrong with it*/
...
void spell_chin_kang_palm(int sn, int level, CHAR_DATA *ch, void *vo)
{
  damage(ch, (CHAR_DATA *) vo, dice(3, 8) + (level / 2) - 6, sn);
}
