...
bool	paradox_gain	args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
bool	status_gain	args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	...
	ch->stance[0] < 1 && ch->position != POS_FIGHTING)
    {
	switch ( ch->autostance )
	{
	    ...
	    default: bug("Error interpreting autostance stance.", 0);
	}
    }

    if (!IS_NPC(victim) && IS_SET(victim->extra, EXTRA_STANCE) &&
	victim->stance[0] < 1 && victim->position != POS_FIGHTING)
    {
	switch ( victim->autostance )
	{
	    case STANCE_NONE:     break;
	    case STANCE_VIPER:    do_stance(victim,"viper"); break;
	    case STANCE_CRANE:    do_stance(victim,"crane"); break;
	    case STANCE_CRAB:     do_stance(victim,"crab"); break;
	    case STANCE_MONGOOSE: do_stance(victim,"mongoose"); break;
	    case STANCE_BULL:     do_stance(victim,"bull"); break;
	    case STANCE_MANTIS:   do_stance(victim,"mantis");break;
	    case STANCE_DRAGON:   do_stance(victim,"dragon");break;
	    case STANCE_TIGER:    do_stance(victim,"tiger"); break;
	    case STANCE_MONKEY:   do_stance(victim,"monkey");break;
	    case STANCE_SWALLOW:  do_stance(victim,"swallow"); break;
	    default: bug("Error interpreting autostance stance.", 0);
	}
    }
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	{
            if (ch->pcdata->stats[UNI_GEN] == 4) max_dam = 2800;
            else max_dam = 3000;
            if (ch->in_room->vnum == ROOM_VNUM_HELL &&
		ch->clan[0] != '\0') 
	        max_dam = 10000;
	    else if (ch->in_room->vnum == ROOM_VNUM_HELL &&
		ch->clan[0] == '\0')
		max_dam = 7500;
            max_dam += (ch->pcdata->stats[DEMON_CURRENT] / 500);
	 }
	else if (IS_CLASS(ch, CLASS_VAMPIRE))
	{
	    if      (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 3500;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 3400;
	    else if (ch->pcdata->stats[UNI_GEN] < 5) max_dam = 3300;
	    else                      	             max_dam = 3200;
	    max_dam += (ch->pcdata->stats[UNI_RAGE] * 10);
	    if (IS_VAMPAFF(ch, VAM_POTENCE)) max_dam += 100;
            if (IS_SET(ch->special, SPC_INCONNU)) max_dam +=100;
	...
	{
	    if      (ch->pcdata->stats[UNI_GEN] < 2) max_dam = 3000;
	    else if (ch->pcdata->stats[UNI_GEN] < 3) max_dam = 2900;
	    else if (ch->pcdata->stats[UNI_GEN] < 4) max_dam = 2800;
	    else                      		     max_dam = 2700;
	    if(ch->pcdata->powers[WPOWER_BOAR] > 4) max_dam += 1000;
	...
	{
	    max_dam += (ch->pcdata->powers[HPOWER_TRAINED] * 65);
	}

	/* Whoever has a greater legend gets the bonus, calculated from the
	 * difference between legend levels.  If victim is an NPC, 
	 * then NPC legend is level / 100 with a legend no greater than 10
	 * and no less than 0. - Puck
	 */
	if ( !IS_NPC(victim) )
	    legend_difference = ch->pcdata->legend - victim->pcdata->legend;
	else
	    legend_difference = ch->pcdata->legend - victim->damroll / 100;

	if (legend_difference < 0) 
	    legend_difference = 0;
	else if (legend_difference > 10) 
	    legend_difference = 10;

	switch(legend_difference)
	{
	    case 1: max_dam += 25; break;
 	    case 2: max_dam += 50; break;
	    case 3: max_dam += 150; break;
	    case 4: max_dam += 200; break;
	    case 5: max_dam += 350; break;
	    case 6: max_dam += 550; break;
	    case 7: max_dam += 900; break;
	    case 8: max_dam += 1450; break;
	    case 9: max_dam += 2350; break;
	    case 10: max_dam += 3800; break;
	    default: max_dam += 0; break;
	}
    ...
    max_dam += ch->damcap[DAM_MOD];
    if (max_dam < 2000) max_dam = 2000;
...
void damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if (IS_CLASS(ch,CLASS_DEMON) && ch->in_room->vnum == ROOM_VNUM_HELL)
	    dam *= 2.25;
	if (!IS_CLASS(ch,CLASS_DEMON) && ch->in_room->vnum == ROOM_VNUM_HELL)
	    dam *= .75;
...
/* Weapon resistances */
    recover = 0;
/* Slash, slice immunity */
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_SLASH) &&
                (dt == TYPE_SLASH || dt == TYPE_SLICE) && dam > 0 )
        recover = number_range(1,dam);

/* Stab, pierce immunity */
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_STAB) &&
                (dt == TYPE_STAB || dt == TYPE_PIERCE) && dam > 0 )
        recover = number_range(1,dam);

/* Blast, pound, crush immunity */
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_SMASH) &&
                (dt == TYPE_BLAST || dt == TYPE_POUND || dt == TYPE_CRUSH) && dam > 0 )
        recover = number_range(1,dam);

/* Bite, claw immunity */
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_ANIMAL) &&
                (dt == TYPE_BITE || dt == TYPE_CLAW) && dam > 0 )
        recover = number_range(1,dam);

/* Grep, suck, whip immunity */
    if ( !IS_NPC(victim) && IS_IMMUNE(victim, IMM_MISC) &&
                (dt == TYPE_GREP || dt == TYPE_SUCK || dt == TYPE_WHIP) && dam > 0 )
        recover = number_range(1,dam);
    dam -= recover;
    
/* Fortitude, etc for non-magical damage */
    recover = 0;
    if ( dt >= TYPE_HIT || dt == gsn_kick )
    {
    	if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_VAMPIRE) &&
                IS_VAMPAFF(victim, VAM_FORTITUDE) && dam > 0)
            recover = number_range(1,dam);

    	else if (IS_ITEMAFF(victim, ITEMA_RESISTANCE) && dam > 0)
            recover = number_range(1,dam);

    	else if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_DEMON)
		&& dam > 0 && IS_DEMPOWER( victim, DEM_TOUGH))
            recover = number_range(1,dam);
    }
    else	/* dt < TYPE_HIT (magical damage) */
    {
	if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_COLD) && dam > 0 && 
	( dt == gsn_frost_breath || dt == gsn_chill_touch ))
	    recover = dam;
	if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_HEAT) && dam > 0 &&
	( dt == gsn_fire_breath || dt == gsn_fireball || 
	  dt == gsn_burning_hands || dt == gsn_flamestrike ) )
	    recover = dam;
	if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_LIGHTNING) && dam > 0 &&
	( dt == gsn_shocking_grasp || dt == gsn_lightning_breath ||
	  dt == gsn_call_lightning || dt == gsn_lightning_bolt ))
	{
	...
	( dt == gsn_acid_breath || dt == gsn_acid_blast ))
	   recover = dam;
	if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_DRAIN) && dam > 0 &&
	( dt == gsn_edrain ))
	    recover = dam;
    }
    dam -= recover;
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam )
{
		...
		if ((vnum = victim->pIndexData->vnum) > 29000)
		{
			...
			if (ch->pcdata->stats[DEMON_CURRENT] > 500000)
				ch->pcdata->stats[DEMON_CURRENT] = 500000;
	                if (ch->pcdata->stats[DEMON_TOTAL] > 500000)
				ch->pcdata->stats[DEMON_TOTAL] = 500000;
			break;
		    case 29600:
		    case 30001:
		    case 30006:
    ...
    /*
     * Take care of link dead people. But leave paradoxed people alone.
     */
    if ( !IS_NPC(victim) && !IS_AFFECTED(victim, AFF_PARADOX) && victim->desc == NULL )
    {
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	...
	if (IS_VAMPAFF(ch, VAM_CELERITY) && 
	    ...
	    ch->pcdata->powers[WPOWER_MANTIS] < 6 )
	    chance -= (ch->pcdata->powers[WPOWER_MANTIS] * 5);
	else if (IS_CLASS(ch, CLASS_HIGHLANDER) &&
	    ch->pcdata->powers[HPOWER_TRAINED] >= 3 &&
	    IS_ITEMAFF(ch, ITEMA_HIGHLANDER) )
	    chance -= (ch->pcdata->powers[HPOWER_TRAINED] * 1.5);
/*
 *	if (ch->pcdata->legend == 1) chance -= 1;
 *	if(ch->pcdata->legend == 2) chance -= 2;
 *	if(ch->pcdata->legend == 3) chance -= 3;
 *	if(ch->pcdata->legend == 4) chance -= 4;
 *	if(ch->pcdata->legend == 5) chance -= 5;
 *	if(ch->pcdata->legend == 6) chance -= 6;
 *	if(ch->pcdata->legend == 7) chance -= 7;
 *	if(ch->pcdata->legend == 8) chance -= 8;
 *	if(ch->pcdata->legend == 9) chance -= 9;
 *	if(ch->pcdata->legend == 10) chance -= 10;
 */
    }
    if (!IS_NPC(victim))
    {
	if (IS_VAMPAFF(victim, VAM_CELERITY) && 
	    ...
	    chance += 12;
	if (victim->pcdata->legend == 1) chance += 1;
	if(victim->pcdata->legend == 2) chance += 2;
	if(victim->pcdata->legend == 3) chance += 3;
	if(victim->pcdata->legend == 4) chance += 4;
	if(victim->pcdata->legend == 5) chance += 5;
	if(victim->pcdata->legend == 6) chance += 6;
	if(victim->pcdata->legend == 7) chance += 7;
	if(victim->pcdata->legend == 8) chance += 8;
	if(victim->pcdata->legend == 9) chance += 9;
	if(victim->pcdata->legend == 10) chance += 10;
    }
	...
	IS_CLASS(ch, CLASS_VAMPIRE))
    {
	if (ch->pcdata->rank == AGE_METHUSELAH) chance -= 8;
	else if (ch->pcdata->rank == AGE_ELDER) chance -= 5;
	...
	IS_CLASS(victim, CLASS_VAMPIRE))
    {
	if (victim->pcdata->rank == AGE_METHUSELAH) chance += 8;
	else if (victim->pcdata->rank == AGE_ELDER) chance += 5;
	else if (victim->pcdata->rank == AGE_ANCILLA) chance += 3;
    }

    if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_HIGHLANDER) &&
	victim->pcdata->powers[HPOWER_TRAINED] >= 7 &&
	IS_ITEMAFF(victim, ITEMA_HIGHLANDER) )
    {
	chance += victim->pcdata->powers[HPOWER_TRAINED]; 
    }

    if (!IS_NPC(victim) && IS_AFFECTED(victim, AFF_WEBBED))
	chance = chance * .66;
...
/*	Penalty to victim trying to dodge a vamp */
    if (!IS_NPC(ch) && IS_VAMPAFF(ch, VAM_CELERITY) && 
	IS_CLASS(ch, CLASS_VAMPIRE))
    {	// Originally -15, -10, -5
	if (ch->pcdata->rank == AGE_METHUSELAH) chance -= 8;
...
/*	Age bonus to vamp who is dodging  */
    if (!IS_NPC(victim) && IS_VAMPAFF(victim, VAM_CELERITY) && 
	IS_CLASS(victim, CLASS_VAMPIRE))
    {
	if (victim->pcdata->rank == AGE_METHUSELAH) chance += 8;
...
/* Addition for 'animate dead' spell */
    if( IS_NPC( victim ) 
  	&& victim->pIndexData->vnum == MOB_VNUM_WALKING_DEAD ) 
    {
    	act("$n's lifeless form turns to dust and blows away.", victim,NULL,NULL, TO_ROOM );
    	extract_char( victim, TRUE );
    	return;
    }
    ...
    victim->pcdata->timer[TIMER_DEATH] = 120;
...
void behead( CHAR_DATA *victim )
{
    ...
    char_from_room(victim);		// What does this accomplish?
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp    = 400 - URANGE( -3, 2 - victim->level, 6 ) * 100;
...
//	gch->alignment  = gch->alignment;	// Pointless?
    }
    else if ( align >  500 )
    {
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    else if ( dam <=  25 ) { vs = " like a wimp";	vp = " like a wimp";	}
    ...
    else if ( dam <= 1500 ) { vs = " insanely hard";vp = " insanely hard";}
    else if ( dam <= 2750 ) { vs = " unbelievably hard";vp = " unbelievably hard";}
    else if ( dam <= 4000 ) { vs = " with ULTIMATE power"; vp = " with ULTIMATE power";}
    else if ( dam <= 5500 ) { vs = " with UNGODLY vengeance";vp = " with UNGODLY vengeance";}
    else { vs = " with UNMERCIFUL strength!"; vp = " with UNMERCIFUL strength!" ;}
...
void disarm( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_DISARM) && !IS_CLASS(ch, CLASS_HIGHLANDER))
	return;
    ...
    act( "#W$n disarms you!#n", ch, NULL, victim, TO_VICT    );
    act( "#WYou disarm $N!#n",  ch, NULL, victim, TO_CHAR    );
    act( "$n disarms $N!",  ch, NULL, victim, TO_NOTVICT );

    obj_from_char( obj );

    /* Loyal weapons come back ;)  KaVir */
    /* Unless a highlander does it... :) Puck */

    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_HIGHLANDER) && number_range(1,2) == 1)
    {
	obj_to_char(obj, victim);
    }
    else if (IS_SET(obj->extra_flags, ITEM_LOYAL) && ( !IS_NPC(victim) ) )
    {
...
void trip( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim))
    {
	if (IS_CLASS(victim, CLASS_WEREWOLF) && (victim->pcdata->powers[WPOWER_BOAR] > 3))
	    return;
...
void do_knock( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->move < cost)
	{
	  send_to_char("You feel too fatigued to knock someone over.\n\r",ch);
	  return;
	}
	if ( ( victim = get_char_room( ch, arg ) ) == NULL )
	{
	...
	{
	  send_to_char("But they are not stanced!\n\r",ch);
	  return;
	}
	if (number_range(1,100) > 20)
	{
	  if (number_range(1,100) > 90)
	      do_stance(ch,"");
	  act("You charge at $N but miss, falling flat on your face.",ch,NULL,victim,TO_CHAR);
	  act("$n charges at you but misses, falling flat on thier face.",ch,NULL,victim,TO_VICT);
	  act("$n charges at $N but misses, falling flat on thier face.",ch,NULL,victim,TO_NOTVICT);
	  cost *= 0.50;
	  ch->move -= cost;
	  WAIT_STATE(ch,12);
	  return;
	}
	act("You charge at $N knocking them from there stance.",ch,NULL,victim,TO_CHAR);
	act("$n charges at you knocking you from your stance.",ch,NULL,victim,TO_VICT);
	act("$n charges at $N knocking them from there stance.",ch,NULL,victim,TO_NOTVICT);
	do_stance(victim,"");
	ch->move -= cost;
	WAIT_STATE(ch,24);
...
void do_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(ch))    cost = (number_range(50,200));
    else		cost = 0;
    ...
    if (ch->move < cost)
    {
	send_to_char( "You're too fatigued to kick anyone.\n\r",ch);
	return;
    }
    if ( ( victim = ch->fighting ) == NULL )
    {
	...
	{
    act("Your kick misses $N.",ch,NULL,victim,TO_CHAR);
    act("$n's kick misses you.",ch,NULL,victim,TO_VICT);
    act("$n's kick misses $N.",ch,NULL,victim,TO_NOTVICT);
	return;
	}
	...
	if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_HIGHLANDER) &&
	    ...
	    number_range(1, 3) == 1)
	{
	    act("You dodge $n's kick with amazing agility.", ch, NULL, victim, TO_VICT);
	    damage (ch, victim, 0, gsn_kick);
	}
	else    
	    damage (ch, victim, dam, gsn_kick);
    }
    else if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_KICK) )
    {
	damage (ch, victim, 0, gsn_kick);
    }
    else
...
void do_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim) && IS_CLASS(victim, CLASS_WEREWOLF) &&
	victim->pcdata->powers[WPOWER_BOAR] > 3)
    {
	store = victim->hit;
	victim->hit += dam;
	damage( ch, victim, dam, gsn_punch );
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
    ...
    CHAR_DATA *mount;
    int number_hit = 0;
    int cost = (number_range(500,1000));
    ...
    if (ch->move < cost)
    {
	send_to_char("You're too fatigued to berserk.\n\r",ch);
	return;
    }
...
void do_disarm( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_HIGHLANDER) && ch->pcdata->powers[HPOWER_TRAINED] >= 6)
    {
    	if (percent < ch->pcdata->learned[gsn_disarm] * 2 / 3 )
	    disarm (ch, victim);
	else
	    send_to_char("You failed.\n\r", ch);
    }
    else if (!IS_NPC(victim) && IS_IMMUNE(victim, IMM_DISARM))
	send_to_char( "You failed.\n\r", ch );
...
void do_parado( CHAR_DATA *ch, char *argument)
{
	send_to_char( "If you want to paradox someone, type PARADOX... spell it out.\n\r",ch);
	return;
}
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( IS_CLASS(ch, CLASS_VAMPIRE) || IS_VAMPAFF(victim, VAM_MORTAL) ) 
	...
	|| IS_SET(victim->in_room->room_flags, ROOM_NO_DECAP))
    {
        send_to_char("You cannot decapitate anyone in this place.\n\r",ch);
        return;
    }
    ...
    pg = paradox_gain(ch, victim);
    if (!pg)
	sg = status_gain(ch, victim);
	...
	sprintf( log_buf, "%s decapitated by %s for no status.",
	    victim->name, ch->name );
    }
...
bool status_gain ( CHAR_DATA *ch, CHAR_DATA *victim )
{
    bool gained = FALSE;
    int chpts = ch->race + ch->pcdata->legend;
    int victpts = victim->race + victim->pcdata->legend;
    ...
    if ( (victim->pcdata->legend == 10 && victim->race > 0)
	 || victim->race >= 25)
    {
	gained = TRUE;
    }
    else if ( (victpts >= 15) && (chpts < (victpts + 15))  )
    {
	gained = TRUE;
    }
    else if ( (victpts < 15) && (chpts < (victpts + 10) ) )
    {
	gained = TRUE;
    }
    ...
    if (victim->race < 0) victim->race = 0;
    if (ch->race > 55) ch->race = 55;

    return gained;
}

bool paradox_gain ( CHAR_DATA *ch, CHAR_DATA *victim )
{
    int chpts, victpts;

    chpts = ((ch->race / 2) + (ch->pcdata->legend / 2));
    victpts = (victim->race + victim->pcdata->legend);

    if ( carrying_quest_bit(victim, QUEST_ARTIFACT) )
    {
	return FALSE;
    }

    if ( victim->race >= 25 || victim->pcdata->legend == 10 )
    {
	return FALSE;
    }
    ...
    else if ( chpts > victpts && ch->race > 15 )
    {
	ch->paradox[1] += (chpts - victpts);
	return TRUE;
    }
