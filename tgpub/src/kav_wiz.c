...
*/
	...
	send_to_char( "Save v spell.............. max =  -5,      at  30 points per point.\n\r",ch);
	send_to_char( "- - - - - - - - - - ----====[ QUEST ITEM COSTS ]====---- - - - - - - - - - -\n\r",	 ch );
	return;
    }
...
/*	if ((ch->pcdata->legend < 10) && (ch->level < 8))
	{
	    send_to_char("You need Almighty Legend Status first.\n\r",ch );
	    return;
	}*/
	if (IS_SET(obj->quest, QUEST_RELIC))
	{
	...
	{
	    send_to_char("What the hell are you doing?\r\n", ch);
	    sprintf(logbuf, "Log %s: Questing negative power on a weapon.", ch->name);
	    log_string(logbuf, 0);
	    return;
	}
...
*/
    ...
    if (obj->item_type != ITEM_BOOK)
    {
	    ...
	    {oset_affect(ch,obj,value,APPLY_SAVING_SPELL,TRUE);return;}
    }
...
*/
	...
	{
	    if (strlen(buf)+strlen(arg3) >= 1800)
	    {
...
*/
    ...
    else if ( affect == APPLY_SAVING_SPELL    ) {range = 5; cost = 30 ; quest = QUEST_SAVING_SPELL;}
    else return;

    if (IS_SET(obj->quest, QUEST_IMPROVED))
    {
	max = 1250 + (ch->race * 20);
	if (max > 1750) max = 1750;
    }
