...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	fprintf( fp, "TimerData	");
	    for (sn = 0; sn < 5; sn++)
	    {
		fprintf(fp, "%d ", ch->pcdata->timer[sn]);
	    }
	    fprintf(fp, "\n");
...
/*
    ...
    /*
     * Don't save artifacts. Quit looks for them and leaves one in the game.
     */
...
void fread_char_old( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
       ...
       KEY( "Autostance",       ch->autostance,      fread_number( fp ) );
	    break;
	...
	case 'L':
	    ...
	    {
	    	ch->pcdata->legend = fread_number( fp );
		fMatch = TRUE;
		break;
	    }
...
*/
	...
	case 'T':
	    ...
	    if ( !str_cmp( word, "TimerData" ) )
	    {
		ch->pcdata->timer[0]	= fread_number( fp );
		ch->pcdata->timer[1]	= fread_number( fp );
		ch->pcdata->timer[2]	= fread_number( fp );
		ch->pcdata->timer[3]	= fread_number( fp );
		ch->pcdata->timer[4]	= fread_number( fp );
		fMatch = TRUE;
		break;
	    }
...
void fread_obj( CHAR_DATA *ch, FILE *fp )
{
    ...
    *obj		= obj_zero;
	...
	case 'O':
	    KEY( "OID",		obj->oid,		fread_number( fp ) );
	    break;
...
/*		ch->pcdata->powers[HPOWER_WPNSKILL] =
ch->pcdata->disc_a[0]; */
...
void fwrite_room_save( ROOM_INDEX_DATA *room )
{
    FILE *fp;
    char buf[MAX_INPUT_LENGTH];
    int x, sysreturn;
     ...
     * file called SAVE(vnum) located in the saveroom
     * directory.  This will make sure that each room
...
//    }
    ...
    {
      bug( "Cannot open ROOM SAVE file for write.\n\r", 0 );
      return;
    }

    fwrite_room_obj( room, room->contents, fp, 0 );
    for ( x = 0; x < 51; x++ )
      fprintf( fp, "XXX\n\n" );
...
void fwrite_room_obj( ROOM_INDEX_DATA *room, OBJ_DATA *obj, FILE *fp, int iNest )
{
    EXTRA_DESCR_DATA *ed;
    AFFECT_DATA *paf;

    /*
     * I slightly edited the fwrite_obj function used
     * to save characters eq and inventory to make use
     * of it to save eq in certain rooms with the ROOM_SAVE_EQ
     * flag.  I am bored and hope this will increase interest
     * in the mud. -- Xodin
     */

    /* Writes items from last to first */
    if ( obj == NULL )
      return;
    if ( obj->next_content != NULL )
	fwrite_room_obj( room, obj->next_content, fp, iNest );

   /*
    * Damn hoodlems always try to get away with cheap
    * shit.  Not this time :P  -- Xodin
    */
   ...
   /*
    * Your general save_obj routine -- Xodin
    */
    ...
    if ( obj->contains != NULL )
	fwrite_room_obj( room, obj->contains, fp, iNest + 1 );
...
void load_room_obj( )
{
  ...
  for ( iScan = 0; iScan < 32000; iScan++ )
  {
    ...
    if ( IS_SET( room->room_flags, ROOM_SAVE_EQ ) )
    {
      sprintf( buf, "../saveroom/save%d", room->vnum );
...
void fread_room_obj( ROOM_INDEX_DATA *room, FILE *fp )
{
/*
    static OBJ_DATA obj_zero;
*/
