...
void	load_area	args( ( FILE *fp, char *filename ) );
...
void	reset_area	args( ( AREA_DATA * pArea, bool fFirstLoad ) );
...
void boot_db( void )
{
	...
	if ( NULL == (fOid = fopen(OID_FILE,"r") ) )
	{
	    perror("Can't open OID file.");
	    exit(1);
	}

	oid_last = fread_number(fOid);
	fclose(fOid);
    }
	...
	fBootDb	= FALSE;
      load_room_obj( );
	area_update( TRUE );	// TRUE, this is the 1st loading of the area
	load_boards( );
//	load_notes( );
	load_ban( );
	load_disabled();
 ...
 */
void load_area( FILE *fp, char *filename )
{
    AREA_DATA *pArea;
    ...
    pArea->author	= strdup (fread_word( fp ));
...
void load_mobiles( FILE *fp )
{
	...
	pMobIndex->description		= fread_string( fp );
      pMobIndex->spec_name          = str_dup( "" );
	pMobIndex->long_descr[0]	= UPPER(pMobIndex->long_descr[0]);
	pMobIndex->description[0]	= UPPER(pMobIndex->description[0]);
      pMobIndex->area_from          = area_last;
...
void load_rooms( FILE *fp )
{
	...
	for ( ; ; )
	{
	    ...
	    {
		bug( "Load_rooms: vnum %d has flag not 'DEST'.", vnum );
		exit( 1 );
	    }
	}
 ...
 */
void area_update( bool fFirstLoad )
{
    AREA_DATA *pArea;

    /*
     * If you can find a less processor consuming way
     * to do this....please inform me.  However, it
     * shouldn't be too bad -- Xodin
     */

    ROOM_INDEX_DATA *pRoom;
    int scanI;

    for ( scanI = 0; scanI < 32000; scanI++ )
    {
      if ( ( pRoom = get_room_index( scanI ) ) == NULL )
         continue;

      if ( pRoom->contents == NULL )
         continue;

      if ( !IS_SET( pRoom->room_flags, ROOM_SAVE_EQ ) )
         continue;

      fwrite_room_save( pRoom );
    }

   /*
    * End of Xodin room saving stuff
    */
 ...
 * Reset one area.
 */
void reset_area( AREA_DATA *pArea, bool fFirstLoad )
{
    RESET_DATA *pReset;
    CHAR_DATA *mob;
    bool last;
	...
	case 'U':
	  if (fFirstLoad)
	  {
	    if ( ( pObjIndex = get_obj_index( pReset->arg1 ) ) == NULL )
	    {
		bug( "Reset_area: 'U': bad vnum %d.", pReset->arg1 );
		continue;
	    }
	...
	case 'X':
	  if (fFirstLoad)
	  {
	    if ((pObjIndex = get_obj_index( pReset->arg1)) == NULL)
	    {
		bug("Reset_area: 'X': bad vnum %d.", pReset->arg1 );
		continue;
	    }
	    ...
	    if ( mob == NULL )
	    {
		bug( "Reset_area: 'X': null mob for vnum %d.",
		    pReset->arg1 );
		last = FALSE;
		break;
	    }
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    mob->armor		= interpolate( mob->level, 100, -300 );

    tempvalue		= mob->level * 12 + number_range(
				mob->level * mob->level / 4,
				mob->level * mob->level );
    if (tempvalue > 32000) mob->max_hit = 32000;
...
/* Freeing hqinfo */
	...
	{
	    if (ch->pcdata->hqinfo->filename != NULL) 
		free_string( ch->pcdata->hqinfo->filename );

	    if (ch->pcdata->hqinfo->obj_name != NULL)
		free_string( ch->pcdata->hqinfo->obj_name );
	    if (ch->pcdata->hqinfo->obj_short_descr != NULL)
		free_string( ch->pcdata->hqinfo->obj_short_descr );
	    if (ch->pcdata->hqinfo->obj_description != NULL)
		free_string( ch->pcdata->hqinfo->obj_description );
	    if (ch->pcdata->hqinfo->obj_chpoweron != NULL)
		free_string( ch->pcdata->hqinfo->obj_chpoweron );
	    if (ch->pcdata->hqinfo->obj_chpoweroff != NULL)
		free_string( ch->pcdata->hqinfo->obj_chpoweroff );
	    if (ch->pcdata->hqinfo->obj_chpoweruse != NULL)
		free_string( ch->pcdata->hqinfo->obj_chpoweruse );
	    if (ch->pcdata->hqinfo->obj_victpoweron != NULL)
		free_string( ch->pcdata->hqinfo->obj_victpoweron );
	    if (ch->pcdata->hqinfo->obj_victpoweroff != NULL)
		free_string( ch->pcdata->hqinfo->obj_victpoweroff );
	    if (ch->pcdata->hqinfo->obj_victpoweruse != NULL)
		free_string( ch->pcdata->hqinfo->obj_victpoweruse );

	    if (ch->pcdata->hqinfo->mob_player_name != NULL)
		free_string( ch->pcdata->hqinfo->mob_player_name );
	    if (ch->pcdata->hqinfo->mob_short_descr != NULL)
		free_string( ch->pcdata->hqinfo->mob_short_descr );
	    if (ch->pcdata->hqinfo->mob_long_descr != NULL)
		free_string( ch->pcdata->hqinfo->mob_long_descr );
	    if (ch->pcdata->hqinfo->mob_description != NULL)
		free_string( ch->pcdata->hqinfo->mob_description );
	    if (ch->pcdata->hqinfo->spec_name != NULL)
		free_string( ch->pcdata->hqinfo->spec_name );
	    if (ch->pcdata->hqinfo->reset_char != NULL)
		free_string( ch->pcdata->hqinfo->reset_char );

	    if (ch->pcdata->hqinfo->room_name != NULL)
		free_string( ch->pcdata->hqinfo->room_name );
	    if (ch->pcdata->hqinfo->room_description != NULL)
...
char *fread_word( FILE *fp )
{
	...
	*pword = getc( fp );
    ...
    sprintf(bugbuf, "Fread_word: word too long: %s", word);
...
*/
	sprintf( buf, "%-10s%-29s%-10s%-29s\n\r",
pArea1->author, pArea1->name, (pArea2 != NULL) ? pArea2->author : "",
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Oid	   %5d\n\r", oid_last	   ); send_to_char( buf, ch );
 ...
 */
void log_string( const char *str, int val )
{
    char *strtime;
...
void generate_room( CHAR_DATA *ch )
{
  ROOM_INDEX_DATA *pRoomIndex;
  int iHash;
  int door;

  ch->pcdata->hqinfo->vnum = free_hq_vnum( ch, "room" );
  ...
  pRoomIndex->vnum		= ch->pcdata->hqinfo->vnum;
  pRoomIndex->name		= str_dup( "A room" );
  pRoomIndex->description	= str_dup( "You are standing in a room.\n\r" );
  pRoomIndex->room_flags	= ROOM_INDOORS|ROOM_NO_TELEPORT;
  pRoomIndex->sector_type	= SECT_HQ;
  pRoomIndex->light		= 0;
  pRoomIndex->blood		= 0;
  pRoomIndex->roomtext		= NULL;
  for ( door = 0; door <= 4; door++ )
  ...
  iHash			= ch->pcdata->hqinfo->vnum % MAX_KEY_HASH;
  pRoomIndex->next	= room_index_hash[iHash];
  room_index_hash[iHash]	= pRoomIndex;
  top_room++;
}

void generate_object( CHAR_DATA *ch )
{
  OBJ_INDEX_DATA *pObjIndex;
  OBJ_DATA *obj;
  ...
  vnum = free_hq_vnum( ch, "object" );

  pObjIndex = alloc_perm(sizeof(*pObjIndex));

  pObjIndex->vnum			= vnum;
	// Name object and inform creator of name 
  sprintf( buf, "%c%c%c", number_range( 97, 122), number_range( 97, 122), number_range( 97, 122) ); 
  send_to_char("Remember this! #yObject name:#n#Y ", ch);
  send_to_char(buf, ch);
  send_to_char("#n\r\n", ch);

  pObjIndex->name			= str_dup( buf );
  pObjIndex->short_descr	= str_dup( "an object" );
  pObjIndex->description	= str_dup( "An object lies here collecting dust." );

  pObjIndex->short_descr[0]	= LOWER(pObjIndex->short_descr[0]);
  pObjIndex->description[0]	= UPPER(pObjIndex->description[0]);

  pObjIndex->area_from          = ch->in_room->area;
  ...
  pObjIndex->value[2]		= 0;
  pObjIndex->value[3]		= 0;
  pObjIndex->weight		= 500;
  pObjIndex->cost		= 0;
  pObjIndex->affected		= NULL;
  pObjIndex->extra_descr	= NULL;
  pObjIndex->chpoweron		= str_dup( "(null)" );
  pObjIndex->chpoweroff		= str_dup( "(null)" );
  pObjIndex->chpoweruse		= str_dup( "(null)" );
  pObjIndex->victpoweron	= str_dup( "(null)" );
  pObjIndex->victpoweroff	= str_dup( "(null)" );
  pObjIndex->victpoweruse	= str_dup( "(null)" );
  pObjIndex->spectype		= 0;
  pObjIndex->specpower		= 0;
  ...
  pReset->command 		= 'O';
  pReset->area_from		= ch->in_room->area;
...
void generate_mobile( CHAR_DATA *ch )
{
  MOB_INDEX_DATA *pMobIndex;
  int vnum;
  int iHash;

  vnum = free_hq_vnum( ch, "mobile" );

  pMobIndex = alloc_perm(sizeof(*pMobIndex));

  pMobIndex->vnum			= vnum;
  pMobIndex->player_name	= ch->pcdata->hqinfo->mob_player_name;
  pMobIndex->short_descr	= ch->pcdata->hqinfo->mob_short_descr;
  pMobIndex->long_descr		= ch->pcdata->hqinfo->mob_long_descr;
  pMobIndex->description	= ch->pcdata->hqinfo->mob_description;
  pMobIndex->spec_name        = str_dup( "" );
  pMobIndex->long_descr[0]	= UPPER(pMobIndex->long_descr[0]);
  pMobIndex->description[0]	= UPPER(pMobIndex->description[0]);
  pMobIndex->area_from        = area_last;
  pMobIndex->act			= ch->pcdata->hqinfo->mob_act | ACT_IS_NPC;
  pMobIndex->affected_by	= ch->pcdata->hqinfo->mob_affected_by;
  pMobIndex->itemaffect		= 0;
  pMobIndex->pShop		= NULL;
  pMobIndex->alignment		= ch->pcdata->hqinfo->mob_alignment;
  pMobIndex->level		= ch->pcdata->hqinfo->mob_level;
  ...
  pMobIndex->damplus		= 0;
  pMobIndex->gold			= 0;
  pMobIndex->sex			= ch->pcdata->hqinfo->mob_sex;
...
void generate_exit( CHAR_DATA *ch )
{
  EXIT_DATA *exit;
  exit = alloc_perm( sizeof(*exit));
  exit->to_room = get_room_index( ch->pcdata->hqinfo->obj_specpower );
  exit->vnum = ch->pcdata->hqinfo->obj_specpower;
  exit->key = -1;
  exit->exit_info = 0;
  exit->keyword = str_dup( "" );
  exit->description = str_dup( "" );
  ch->in_room->exit[ch->pcdata->hqinfo->vnum] = exit;
}

unsigned int generate_oid( OBJ_DATA *obj )
{
    if (obj == NULL)
    {
	return -1;
