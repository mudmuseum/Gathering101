...
bool too_many = FALSE;	/* Flag to cut out of get all loop - be sure to
			 * reinitialize to FALSE before a function returns */
bool too_much = FALSE;
...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    if ( IS_SET(obj->quest, QUEST_ARTIFACT) || contains_quest_bit(obj, QUEST_ARTIFACT) )
    {
	if ( carrying_quest_bit(ch, QUEST_ARTIFACT) )
	{
	    act( "You already have an artifact!", ch, NULL, NULL, TO_CHAR);
	    return;
	}

	if ( ch->level < LEVEL_AVATAR || IS_NPC(ch) )
	{
	    act( "That object is too powerful for you.", ch, NULL, NULL, TO_CHAR);
	    return;
	}
    }
...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		&&   can_see_obj( ch, obj ) )
		{
		    ...
		    {
			too_many = FALSE; /* Reinitialize before returning */
			return;
		    }
		}
	    }
...
*/
    ...
    if ( IS_SET( ch->in_room->room_flags, ROOM_SAVE_EQ ) )
      fwrite_room_save( ch->in_room );
...
*/
    ...
    {
	send_to_char( "You cannot put that in anything.\n\r", ch );
	return;
    }
	...
	if (IS_SET(obj->quest, QUEST_UNIQUE) )
	{
	    send_to_char("You cannot put unique items in a container.\n\r",ch);
	    return;
	}
...
void do_drop( CHAR_DATA *ch, char *argument )
{
	...
	if ( !can_drop_obj( ch, obj ) )
	{
	    ...
	    {
		act( "$p magically falls to the floor.", ch, obj, NULL, TO_ROOM );
		act( "$p magically falls to the floor.", ch, obj, NULL, TO_CHAR );
		return;
	    } 
	    else 
	    {
		act( "$p magically disappears.", ch, obj, NULL, TO_CHAR );
	    	act( "$p magically disappears.", ch, obj, NULL, TO_ROOM );
	    	act( "$p magically appears.", NULL, obj, NULL, TO_OBJ );
		return;
	    }
    	}
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(obj->quest, QUEST_ARTIFACT) || contains_quest_bit(obj, QUEST_ARTIFACT))
    {
      /* Victim already has an arti, or is an NPC, or is a mortal */
	if ( carrying_quest_bit(victim, QUEST_ARTIFACT) || IS_NPC(victim) || 
		victim->level < LEVEL_AVATAR)
	{
	    act ( "That object is too powerful for them.", ch, NULL, NULL, TO_CHAR );
	    return;
	}
      /* Victim can hold an arti, but needs to be in safe-from-decap place */
	if ( ch->in_room->sector_type != SECT_HQ &&
		!IS_SET(ch->in_room->room_flags, ROOM_NO_DECAP) &&
		!IS_SET(ch->in_room->room_flags, ROOM_SAFE) )
	{
	    send_to_char ( "You should find a safe place to exchange artifacts.\r\n", ch);
	    return;
	}
    }
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	value = obj->value[0];
        sprintf( log_buf, "%s eats a %d point quest token.", ch->name, value );
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
	...
	if (IS_ITEMAFF(ch, ITEMA_UNIQUE) && IS_SET(obj->quest, QUEST_UNIQUE) )
	{
	    send_to_char("You cannot use more than one unique item.\n\r",ch);
	    return;
	}
...
void do_wear( CHAR_DATA *ch, char *argument )
{
	...
	{
	    obj_next = obj->next_content;
    if (ch->fighting != NULL && ( ( obj->pIndexData->vnum == 29511) ||  (obj->pIndexData->vnum == 29517) ) )
    {
        send_to_char( "That item is too powerful for you to equip while fighitng.\n\r", ch);
        continue;
    }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	...
	act( buf, ch, obj, NULL, TO_CHAR );
        sprintf( log_buf, "%s gets a %d qp refund from sacing %s.", ch->name, obj->points, obj->short_descr);
        log_string( log_buf, obj->oid );
	ch->pcdata->quest += obj->points;
    }

    /*
     * Corpses and containers spill their important contents when sacrificed.
     */
    if ( (obj->item_type == ITEM_CORPSE_NPC || obj->item_type == ITEM_CONTAINER)
	&& obj->contains != NULL)
    {
	for ( obj2 = obj->contains; obj2 != NULL; obj2 = obj_next )
	{
            obj_next = obj2->next_content;
	    if ( IS_SET(obj2->quest, QUEST_ARTIFACT) || 
		 IS_SET(obj2->quest, QUEST_UNIQUE) ||
		 obj2->item_type == ITEM_CORPSE_NPC ||
		 obj2->item_type == ITEM_CONTAINER ||
	       ( obj2->questowner != NULL && strlen(obj2->questowner) > 1 ) )
	    {
            	obj_from_obj( obj2 );
	    	obj_to_room( obj2, obj->in_room);
		act( "$p falls out of $P.", ch, obj2, obj, TO_OBJ );
	    }
	}
    }
    extract_obj( obj );
...
int get_cost( CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
	...
	cost = cost * obj->value[2] / (obj->value[1] ? obj->value[1] : 1);
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(obj->quest, QUEST_ARTIFACT) || IS_SET(obj->quest, QUEST_UNIQUE) )
    {
	act( "$n looks uninterested in $p.", keeper, obj, ch, TO_VICT );
	return;
    }
...
void do_activate( CHAR_DATA *ch, char *argument )
{
	...
	do_look(ch,"auto");
       ...
       if (!IS_SET(obj->quest, QUEST_ARTIFACT) &&
            ...
            CAN_WEAR(obj,ITEM_TAKE))
        {
            ...
            char_from_room(ch);
            char_to_room(ch,get_room_index(17381));
    ...
    else if ( IS_SET(obj->spectype, SITEM_TELEPORTER))
    {
        ...
        send_to_char("Your transporter explodes, sending you to the ground stunned!\n\r",ch);
        act("$n's transporter explodes, sending them to the ground stunned!", ch, NULL, NULL, TO_ROOM);
        sprintf( buf, "%s is rocked by a terrible explosion!", ch->name );
        do_info(ch,buf);
        obj->specpower = 0;
        return;
        }
	...
	if (obj->victpoweroff != NULL && str_cmp(obj->victpoweroff,"(null)") &&
	    obj->victpoweroff != '\0')
	{
		    if (( IS_IMMORTAL(victim)) || (victim->level <=2))  return;
...
void do_twist( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( IS_SET(obj->spectype, SITEM_TELEPORTER))
    {
	    ...
	    && str_cmp(obj->victpoweron,"(null)") )
	    kavitem(str_dup(obj->victpoweron),ch,obj,NULL,TO_ROOM);
        if (!IS_SET(obj->quest, QUEST_ARTIFACT) && (number_range(1,15) ==1) 
	    && CAN_WEAR(obj, ITEM_TAKE))
        {
        ch->hit *= 0.5;
        ch->position = POS_STUNNED;
...
void do_recharge( CHAR_DATA *ch, char *argument )
{
	...
	return;
    	...
    	if (IS_AFFECTED(ch, AFF_SHADOWPLANE))
    	{
	    send_to_char("You cannot do that while in the shadowplane.\n\r",ch);
	    return;
    	}
    ...
    if (number_range(1,20) == 1 && value <= 100) 
    {
        sprintf( log_buf, "The Quest Machine keeps %s's Quest Card!", ch->name);
	log_string( log_buf, obj->oid );
	extract_obj( obj );
	act("$P doesn't like $p so it decides to keep it.",ch,obj,qobj,TO_CHAR);
	act("$P doesn't like $p so it decides to keep it.",ch,obj,qobj,TO_ROOM);
    }
    else
    {
    ...
    tobj = create_object(get_obj_index( OBJ_VNUM_PROTOPLASM ), 0);
    tobj->value[0] = value;
    tobj->level = value;
    tobj->cost = value*1000;
    tobj->item_type = ITEM_QUEST;
    obj_to_char(tobj,ch);
    tobj->questmaker = str_dup(ch->name);
    free_string(tobj->name);
    tobj->name = str_dup( "quest token" );
    free_string( tobj->short_descr );
    sprintf(buf,"a %d point quest token",value);
    tobj->short_descr = str_dup( buf );
    free_string( tobj->description );
    sprintf(buf,"A %d point quest token lies on the floor.",value);
    tobj->description = str_dup( buf );
    act("You take $p from $P.",ch,tobj,qobj,TO_CHAR);
    act("$n takes $p from $P.",ch,tobj,qobj,TO_ROOM);
    if (!IS_NPC(ch))
    {
	ch->pcdata->score[SCORE_NUM_QUEST]++;
	ch->pcdata->score[SCORE_QUEST] += value;
	sprintf(buf,"%s has completed a quest!.",ch->name);
        sprintf( log_buf, "%s has completed a quest for %d quest points.", ch->name, value );
        log_string( log_buf, tobj->oid);
    }
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    static const sh_int quest_selection[] = 
    {
	 ...
	 6010, 1304, 1307, 1332, 1333, 1342, 1356, 1361, 9016, 2322,
	 10409,2382, 8003, 8005, 5300, 5302, 5309, 5310, 5311, 4000,
	 ...
	 1394, 2202, 9015, 6005, 1614,  647, 1388, 9311, 3604, 4701,
	30325, 6106, 2003, 7190, 9322, 1384, 3412, 2342, 1374, 2210,
	 2332, 2901, 7200, 7824, 3410, 2013, 1510, 8306, 3414, 2005,
	28503,28557,28529,28518,10235,10264,10284,10447,21026, 2430,
	10003,12705,12708,30600,30602,17403,17404,17418,18002,18014,
	   12,   14,   15,30005,30012,30013,30025,30026,30029,30030
    };
    int object;
    ...
    for (i = 0; i < 4; i++)
    {
	object = number_range(1, 180);
        if (object < 1 || object > 180) object = 0;
...
void do_complete( CHAR_DATA *ch, char *argument )
{
    ...
    for (i = 0; i < 4; i++)
	if (qobj->value[i] == -1) numCompleted++;
	...
	for (i = 0; i < 4; i++)
	{
		...
		if ((pObjIndex->vnum >= 10 && pObjIndex->vnum <= 16) || 
		    (pObjIndex->vnum >= 30004 && pObjIndex->vnum < 30037))
		{
		    sprintf(desc, pObjIndex->short_descr, "something");
		}
		else
		    strcpy(desc, pObjIndex->short_descr);
	...
	if (qobj->value[i] != -1)
	{
	    pObjIndex = get_obj_index( qobj->value[i] );
	    if ( pObjIndex != NULL && 
		(pObjIndex->vnum == obj->pIndexData->vnum ||
		 !str_cmp(pObjIndex->short_descr, obj->short_descr)))
	    {
		qobj->value[i] = -1;
