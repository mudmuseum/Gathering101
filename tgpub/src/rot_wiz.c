...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Weapon:  unarmed slice stab slash whip claw blast   \n\r", ch );
	...
	if ( IS_CLASS(victim, CLASS_MAGE) && (ch->level == 6) && (value < 4 || value > 300) )
	{
	    send_to_char( "Spell range is 4 to 300.\n\r", ch );
	    return;
	}
	if ( IS_CLASS(victim, CLASS_MAGE) && (ch->level == 5) && (value < 4 || value > 276) )
	{
	    send_to_char( "Spell range is 4 to 276.\n\r", ch );
	    return;
	}
...
/*
	if ( IS_CLASS(victim, CLASS_WEREWOLF) && (value < 4 || value >
240) && ())
*/
	if ( value < 0 || value > 200 )
	{
...
void paradox( CHAR_DATA *ch )
{
    ...
    do_info(ch, buf);
    sprintf( log_buf, "%s is struck by a paradox.", ch->name );
    log_string( log_buf, ch->paradox[0]);

	/* To prevent paradoxed people from still following their groups */
    if (ch->master != NULL)
	stop_follower(ch);
